declare namespace wx {
  ...
  // 选择图片
  interface chooseImageOptions extends callback<{ errMsg: string; localIds: string[] }> {
    // 允许最多多少张
    count: number;
    // 尺寸类型 原图/压缩图 默认都有
    sizeType: sizeType[];
  ...
  // 上传图片
  interface uploadImageOptions extends callback<{ errMsg: string; serverId: string }> {
  ...
  function invoke(type: string, param: any, callback: (msg: { err_msg: string }) => void): void;
